...
import com.anoopvt.material_calendarview.CalendarDay
...
fun setSelectedDayColorsNew(dayLabel: LinearLayout, calendar: Calendar, calendarProperties: CalendarProperties) {
    val calendarDay = calendarProperties.findDayProperties(calendar)
    ...
    if (calendarDayBackgroundRes != null && !calendarProperties.selectionDisabled) {
        ...
        dayLabel.setDayColors(labelColor, backgroundRes = calendarProperties.selectionBackground)
        tintCelBackground(dayLabel, calendarProperties.selectionColor)
    }
}


fun setSelectedDayCelColors(dayLabel: LinearLayout, calendar: Calendar, calendarProperties: CalendarProperties) {
    val calendarDay = calendarProperties.findDayProperties(calendar)
...
fun setCurrentMonthDayCelColors(calendar: Calendar, dayLabel: LinearLayout?, calendarProperties: CalendarProperties) {
    if (dayLabel == null) return

    setNormalDayCelColors(calendar, dayLabel, calendarProperties)

    if (calendar.isToday) {
        setTodayCelColors(calendar, dayLabel, calendarProperties)
    }

    if (calendar.isEventDayWithLabelColor(calendarProperties)) {
        setEventDayCelColors(calendar, dayLabel, calendarProperties)
    }

    if (calendarProperties.highlightedDays.contains(calendar)) {
        setHighlightedDayCelColors(dayLabel, calendarProperties)
    }
}
...
private fun setTodayCelColors(calendar: Calendar, dayLabel: LinearLayout, calendarProperties: CalendarProperties) {
    val calendarDay = calendarProperties.findDayProperties(calendar)
...
private fun setEventDayCelColors(day: Calendar, dayLabel: LinearLayout, calendarProperties: CalendarProperties) {
    day.getEventDayWithLabelColor(calendarProperties)?.let { eventDay ->
        dayLabel.setDayColors(textColor = eventDay.labelColor)
    }
}
...
private fun setHighlightedDayCelColors(dayLabel: LinearLayout, calendarProperties: CalendarProperties) {
    dayLabel.setDayColors(textColor = calendarProperties.highlightedDaysLabelsColor)
}
...
private fun setNormalDayCelColors(calendar: Calendar, dayLabel: LinearLayout, calendarProperties: CalendarProperties) {
    val calendarDay = calendarProperties.findDayProperties(calendar)
...
private fun tintCelBackground(dayLabel: LinearLayout, color: Int) {
    dayLabel.background.setColorFilter(color, android.graphics.PorterDuff.Mode.MULTIPLY)
}
